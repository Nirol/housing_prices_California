...
import pandas as pd
from src.pipeline.pipeline_constants import CAT_ATTRIBS, NUM_ATTRIBS_TO_IMPUTE, \
...
def rebuild_df_initial_pipeline(dirty_housing,log_num_feature, categorial_features):
    ...
    housing_copy.drop(CAT_ATTRIBS, inplace=True, axis=1)
    housing_copy.reset_index(drop=True, inplace=True)
